...
int main()
{
	...
	while(1)
	{
		...
		{
			input = Xil_In32(XPAR_RC5CONTROLLER_0_BASEADDR);
			input = rc5toInput(input);
			//input = Xil_In32(XPAR_MY_PERIPHERAL_0_BASEADDR);
			//input = (Xil_In32(XPAR_RC5CONTROLLER_0_BASEADDR) == 5) ? 30 : 0;
			//input = Xil_In32(XPAR_RC5CONTROLLER_0_BASEADDR) & 0x000000FF;
			//input = rc5ToKey(Xil_In32(XPAR_RC5CONTROLLER_0_BASEADDR));
			seed++;
		}
		srand(seed);
		/********/
